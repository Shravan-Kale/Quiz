...
public class GameManager : MonoBehaviour {
    ...
    private             bool                quit; // MY Variable
    private             bool                GameOver; // MY Variable
    private             bool                skip; // MY Variable 
    #endregion
    ...
    #region My functions
    /// <summary>
    /// Function that is called to End the game by button.
    /// </summary>
    public void Quit()
    {
        IsFinished = true;
    ...
    /// <summary>
    /// Function that is called to To see the status of the game.
    /// </summary>
    public void Update()
    {
        if (FinishedQuestions.Count == Questions.Length - 1)
            IsFinished = true;
        //if (UpdateScore < 0)
            //quit = true;
    }
    /// <summary>
    /// Function that is called to To skip the game.
    /// </summary>
    public void Skip()
    {
        skip = true;
    ...
    public void Accept()
    {
        if (GameOver)//My defination
            return;//My defination
        bool isCorrect = CheckAnswers();
        UpdateTimer(false);
        if (quit)//My defination
            goto ButtonClicked;//My defination
        
        FinishedQuestions.Add(currentQuestion);

        if (skip)//My defination
        {
            isCorrect = true;//My defination
            goto SKIP;
        }


        UpdateScore((isCorrect) ? Questions[currentQuestion].AddScore : -Questions[currentQuestion].AddScore);
        UpdateScore((quit) ? Questions[currentQuestion].AddScore : 0);
        UpdateScore((skip) ? Questions[currentQuestion].AddScore : 0);
    ButtonClicked :
        if (IsFinished || quit)
        {
            SetHighscore();
        ...
        var type 
            ...
            : (skip) ? UIManager.ResolutionScreenType.Skip : (isCorrect) ? UIManager.ResolutionScreenType.Correct 
            : UIManager.ResolutionScreenType.Incorrect;

            skip = false;//My defination
    ...
    /// <summary>
    /// Function that is called to go to Menu.
    /// </summary>
    public void Menu()
    {
